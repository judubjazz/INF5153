...
public class BattleShipGameController implements GameController {
    ...
    public BattleshipGame replay(BattleshipGame battleshipGame){
        ...
        // TODO Refactor this in one function , maybe playTurn() in BattleshipGame class
        // playerOne turn
...
//            return "you-won";
    ...
    public BattleshipGame load(){
        ...
        BattleshipGame battleshipGame = new BattleshipGame(1,p1,p2,r,false);
    ...
    public BattleshipGame start(String data) {
        ...
        Map<String, Ship> playerOneFleet = ShipsController.buildFleet(playerOneFleetJSON);
        ...
        Map<String, Ship> cpuFleet = AiController.generateFleet(cpuBoard);
        ...
        BattleshipGame battleshipGame = new BattleshipGame(1, player, cpu, recorder, difficulty);
        return  battleshipGame;
    }
    @Override
    public BattleshipGame playTurn(BattleshipGame battleshipGame) {
        ...
        // TODO refactor the code below because it repeats itself, maybe a BattleshipGame class playTurn() function
        int targetedArea = p1.ennemyBoard.map[targetX][targetY];
        if(targetedArea > 0){p2.shipsRemaining--;}
        ...
        // cpu counter attacks
        if(battleshipGame.difficulty){
            // TODO implement minMax algorithm
//            target = AiController.targetMinMaxPosition();
        } else {
            target = AiController.targetRandomPosition();
        }

        while(true){
            if(battleshipGame.recorder.playerTwoMoves.contains(target)){
                target = AiController.targetRandomPosition();
            } else {
